...
Der Browser lädt zusätzlich einen aus dem offiziellen XRechnung-CIUS-Modell generierten Feldkatalog (`web/xrechnung-field-catalog.js`). Dieser Katalog wird nicht als sichtbare Leseliste angezeigt, sondern im Hintergrund an die bestehenden Formularfelder gebunden (`data-bt`, `data-bg`, Tooltips, `(i)`-Hilfefenster und Agenten-API). So bleiben Nutzer im Formularfluss, während Tests und Agenten die XRechnung-Zuordnung maschinenlesbar nutzen können.

Pflichtfelder sind mit `*` markiert; fehlt eines davon, wird nicht konvertiert. Dazu zählen u. a. Rechnungsnummer/-datum/-art, Währung, Leitweg-ID, IBAN/Bankdaten, Zahlungsart, Zahlungsbedingungen, Rechnungssteller-E-Mail/Endpoint-ID, Kontakt-E-Mail/Telefon, Seller Identifier, Käufer-Endpoint/Adressdaten, Auftragsnummer/Bestellreferenz und Positionsdaten inkl. Positions-ID, Menge, Einheit und Nettopreis. Empfehlende Felder wie Leistungs-/Lieferdatum, Kontoinhaber, BIC, USt-ID und Adresszeilen bleiben nicht-blockierend, werden aber mit gleicher `(i)`-Hilfe erklärt. Für andere Agenten/LLMs gibt es `window.XInvoice.convertForAgent(invoice, formatId)` mit strukturierten Fehlern oder Artefakten plus Browser-Validierungsbericht. Zusätzlich stehen `window.XInvoice.getXRechnungFieldCatalog()`, `window.XInvoice.getFieldHelpInfo()`, `window.XInvoice.getRecommendedFields()`, `window.XInvoice.getFormFieldBindings()` und `window.XInvoice.applyXRechnungFieldMetadata(document)` für Feldzuordnung und Formularfüllung bereit.
